...
import com.ex.empt.mapper.EmptMapper;
...
public class EmptService {
	...
	@Autowired
	private EmptMapper mapper;
	
	public List<EmptVO> getEmpList(int deptno){
		...
		if(rows > 0) { //update가 완료됨
			List<EmptVO> list = mapper.selectEmpByDeptno(deptno); //select
	...
	//특정 사원 조회
	public EmptVO getEmpByEmpno(int empno) {
		return mapper.selectEmpByEmpno(empno);
	}
	
	//특정 사원 정보 수정
	public int getEmpUpdate(EmptVO vo) {
